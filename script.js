...
function initWebSocket() {
    ...
    socket.onmessage = e => {
        try {
            ...
            // 🔹 ERPNext orders for admin
            if(currentUser?.role==="admin") loadERPNextWorkOrders();
...
async function loadERPNextWorkOrders(){
    ...
    try {
        ...
        setTimeout(loadERPNextWorkOrders, 5000); // Retry after 5s
    }
}

function renderERPWorkOrders(orders){
    const container = document.getElementById("erpnext-orders");
    if(!container) return;
    container.innerHTML = "<h2>ERPNext Work Orders</h2>";

    if(orders.length === 0){
        container.innerHTML += "<p>No ERPNext work orders.</p>";
        return;
    }
